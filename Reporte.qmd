...
Durante el periodo analizado ( `r periodo_min` al `r periodo_max` ), se notificaron un total de `r total_casos`, de los cuales el `r prop_IRAG`% (`r frec_IRAG`/`r total_casos`) correspondieron a casos de IRAG y el `r prop_IRAG_EXT`% ( `r frec_IRAG_EXT`/ `r total_casos`) a casos de IRAG extendida
...
Del totalde casos notificados, el `r prop_casos_pediatricos`% (`r frec_casos_pediatricos`/`r total_casos_ped_adult`correspondieron a pacientes de edad pediatrica y el `prop_casos_adultos`% restante (`` r frec_casos_adultos` ``/ `r total_casos_ped_adult`) a pacientes adultos.
