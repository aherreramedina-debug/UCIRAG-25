...
```{r}
#| label: txt enriquecido
#| message: false 
#| echo: false
#| warning: false
source("Val_txt_enriq.R")
...
# Análisis de casos notificados en la UC-IRAG del `r establecimiento` - Período: (`r periodo_texto`)

Durante el período analizado (`r periodo_texto`), en el establecimiento `r establecimiento`, se registraron `r total_positivos` casos positivos de Infecciones Respiratorias.

En cuanto a la clasificación manual, hubo: - `r frec_IRAG` casos de IRAG (`r prop_IRAG`%), - `r frec_IRAG_EXT` casos de IRAG extendida (`r prop_IRAG_EXT`%).

La edad media de diagnóstico en pacientes positivos fue de `r round(resumen_edades$media, 1)` años, la mediana de `r resumen_edades$mediana` años, con un desvío estándar de `r round(resumen_edades$desvio_estandar, 1)`. El rango intercuartílico va de `r resumen_edades$p25` a `r resumen_edades$p75` años.
